...
public class GerenciaFaculdade {
    ...
    private static Options definirOpcoesCLI() {
        ...
        options.addOption(Option.builder("vd").longOpt("verifica_Dados").desc("Verifica se os dados estão salvos corretamente no csv.").build());
        options.addOption(Option.builder("aa").longOpt("add_Aluno").desc("Adiciona aluno.").build());
        options.addOption(Option.builder("la").longOpt("lista_Aluno").desc("Lista todos os alunos.").build());
        options.addOption(Option.builder("ea").longOpt("edita_Aluno").hasArg().argName("codigo").desc("Edita o aluno com o código especificado.").build());
        options.addOption(Option.builder("ap").longOpt("add_Professor").desc("Adiciona professor.").build());
        options.addOption(Option.builder("lp").longOpt("lista_Professor").desc("Lista todos os professores").build());
        options.addOption(Option.builder("ep").longOpt("edita_Professor").hasArg().argName("codigo").desc("Edita o professor com o código especificado.").build());
        options.addOption(Option.builder("ac").longOpt("add_Curso").desc("Adiciona curso.").build());
        options.addOption(Option.builder("lc").longOpt("lista_Curso").desc("Lista todos os cursos.").build());
        options.addOption(Option.builder("am").longOpt("add_Materia").desc("Adiciona matéria.").build());
        options.addOption(Option.builder("lm").longOpt("lista_Materia").hasArg().argName("codigo_curso").optionalArg(true).desc("Lista as matérias.").build());
        options.addOption(Option.builder("an").longOpt("add_Nota").numberOfArgs(4).argName("tipo> <codAluno> <codMateria> <nota").desc("Adiciona nota: -an <tipo> <codAluno> <codMateria> <nota>").build());
        options.addOption(Option.builder("fm").longOpt("Fecha_Media").hasArg().argName("codigo_aluno").desc("Calcula e exibe a média final do aluno.").build());
    ...
    private static void executarComando(CommandLine cmd, Options options, HelpFormatter formatter) throws ErroCadastro, IOException, ParseException {
        ...
        if (cmd.hasOption("help")) {
            formatter.printHelp("java br.fsa.GerenciaFaculdade", options);
        } else if (cmd.hasOption("verifica_Dados")) {
            verificarDados();
        } else if (cmd.hasOption("add_Aluno")) {
            adicionarAlunoInterativo();
        } else if (cmd.hasOption("lista_Aluno")) {
            listarAlunos();
        } else if (cmd.hasOption("edita_Aluno")) {
            String codigoAluno = cmd.getOptionValue("edita_Aluno");
            editarAlunoInterativo(codigoAluno);
        } else if (cmd.hasOption("add_Professor")) {
            adicionarProfessorInterativo();
        } else if (cmd.hasOption("lista_Professor")) {
            listarProfessores();
        } else if (cmd.hasOption("edita_Professor")) {
            String codigoProfessor = cmd.getOptionValue("edita_Professor");
            editarProfessorInterativo(codigoProfessor);
        } else if (cmd.hasOption("add_Curso")) {
            adicionarCursoInterativo();
        } else if (cmd.hasOption("lista_Curso")) {
            listarCursos();
        } else if (cmd.hasOption("add_Materia")) {
            adicionarMateriaInterativo();
        } else if (cmd.hasOption("lista_Materia")) {
            String codigoCurso = cmd.getOptionValue("lista_Materia"); //pode ser null se nao fornecido
        ...
        for (Materia materia : materias) {
            if (materia.getCodigoCurso() != null) {
                ...
                if (cursoEncontrado != null) {
                    ...
                    logger.logInfo("Ligando matéria " + materia.getCodigoUnico() + " ao curso " + cursoEncontrado.getCodigoUnico()); //preenche o log
